...
class ContinuousDiffusion(nn.Module):
    ...
    def p_sample(self, x_t: torch.Tensor, t: torch.Tensor, state: torch.Tensor, deterministic: bool = False):
    ...
    def sample(self, state: torch.Tensor, init_x: torch.Tensor = None, noise_scale: float = 1.0, deterministic: bool = False) -> torch.Tensor:
        """
        反向扩散采样：
        - deterministic=True 时采用均值轨迹，不加噪声；用于 actor 训练的可微路径
        - deterministic=False 时，若提供 init_x，则 x_T = init_x + noise_scale * N(0, I)，否则 x_T = noise_scale * N(0, I)
        """
        device = self.betas.device
        batch_size = state.shape[0]
        if deterministic:
            # 确定性：init_x 若提供直接作为初值，否则 0 启动
            x_t = torch.zeros(batch_size, self.action_dim, device=device) if init_x is None else init_x
        else:
            if init_x is None:
        ...
        for i in reversed(range(0, self.n_timesteps)):
            ...
            x_t = self.p_sample(x_t, t, state, deterministic=deterministic)
        return x_t