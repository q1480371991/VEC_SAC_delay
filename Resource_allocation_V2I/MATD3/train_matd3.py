...
batch_size = 64       # 外部触发阈值；实际 update 用 cfg.batch_size
...
def build_reward(shared_global_reward, E_total, Delay_vel, ReplayB_v, overload):
    """
    根据配置生成奖励
    - shared_global: 直接用 -reward_tot
    - shared_from_per_agent: 按权重聚合逐智能体子项，再加全局项
    - per_agent: 返回逐智能体奖励向量（需 CRITIC_VECTOR=True）
    """
...
def save_results(name, index, E_total, reward, calculate, overload, eta1, load_rate_0, delay,
    ...
    # 逐智能体能量/时延曲线（每个智能体在各 episode 的均值）
...
def train_matd3_once(index=0):
    ...
    for i_episode in range(n_episode_test):
        print('------ MATD3/Episode', i_episode, '------')
        env.new_random_game()

        state_old_all = []
        state = env.get_state()   # shape (2*n_veh,)
        ...
        while not time_slots.is_end():
            ...
            reward = build_reward(
                shared_global_reward=reward_tot,
                E_total=E_total,
                Delay_vel=Delay_vel,
                ReplayB_v=env.ReplayB_v,
                overload=overload
            )
